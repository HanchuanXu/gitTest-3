...
public class Filter {
    ...
    public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
        List<Tweet> result = new ArrayList<>();
        for (Tweet tweet : tweets) {
            if (tweet.getTimestamp().getEpochSecond() >= timespan.getStart().getEpochSecond()
                    && tweet.getTimestamp().getEpochSecond() <= timespan.getEnd().getEpochSecond()) {
                result.add(tweet);
            }
        }
        return result;
    ...
    public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
        ...
        for (String word : words) {
            testWords.add(word.toLowerCase());
        }

        for (Tweet tweet : tweets) {
            String[] tweetWords = tweet.getText().split("\\s");
            for (String i : tweetWords) {
                if (testWords.contains(i.toLowerCase())) {
                    result.add(tweet);
                    break;
                }
            }
